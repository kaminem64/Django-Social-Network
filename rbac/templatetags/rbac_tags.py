...
#------------------------------------------------------------------------------------------------------------    
class RBACGenericUserPermissionNode(template.Node):
    def __init__(self, params, nodelist_true, nodelist_false):
        self.nodelist_true, self.nodelist_false = nodelist_true, nodelist_false
        self.owner = params[0]
        self.model = params[1]
        self.operation = params[2]
        self.operator_user = params[3]

    def render(self, context):
        owner = template.resolve_variable(self.owner, context)
        model = template.resolve_variable(self.model, context)
        op = template.resolve_variable(self.operation, context)
        operator_user = template.resolve_variable(self.operator_user, context)
        if RBACGenericPermission.objects.get_user_permission(owner, model, op, operator_user):
            return self.nodelist_true.render(context)
        else:
            return self.nodelist_false.render(context)

@register.tag('if_rbac_generic_user_permission')
def if_rbac_generic_user_permission(parser, token):
    """
    {% if_rbac_generic_user_permission owner model operation operator_user %}
        ...
    {% else %}
        ---
    {% endif_rbac_generic_user_permission %}
    """
    bits = list(token.split_contents())
    end_tag = 'end' + bits[0]
    nodelist_true = parser.parse(('else', end_tag))
    ...
    if token.contents == 'else':
        ...
        nodelist_false = template.NodeList()
    return RBACGenericUserPermissionNode(bits[1:], nodelist_true, nodelist_false)
